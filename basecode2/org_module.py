...
def initialise_admin_account():
	...
	if check_condition_value(ALL_ORG, True):
		...
		super_admin_exists = st.session_state.u_collection.find_one({"username": secrets_retriever.get_secret("super_admin_username")})
		if super_admin_exists:
			...
			st.session_state.u_collection.insert_one({
					...
					"user_id": 0,
					"password": hash_password(secrets_retriever.get_secret("super_admin_password")), #hash_password(SUPER_PWD) 
					"profile": SA,
					"sch_name": ALL_ORG
				})
			insert_condition_value(ALL_ORG, True)
